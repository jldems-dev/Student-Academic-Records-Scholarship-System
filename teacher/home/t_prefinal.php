<table id="tblData" class="table table-bordered table-sm">
    <thead class="text-center">
        <tr>
            <th width="10%"><?php echo $course.'-'.$year;?></th>
            <th colspan="17"></th>
            <th width="1%" colspan="4">Computation</th>
        </tr>
        <tr>
            <th width="1%"><?php echo $subjectcode;?></th>
        ...
        <tr style="background-color: yellow;">
            <?php  for($i = 1; $i <= 5; $i++){
            echo "<th width='1%'>".$rowtl2['pt'.$i]."</th>";
            }?>

            <th width="1%" ><?php echo $pref_tltotal;?></th>

            <?php  for($i = 1; $i <= 5; $i++){
            echo "<th width='1%'>".$rowtl2['quiz'.$i]."</th>";
            }?>

            <th width="1%"><?php echo $pref_tl_quiztotal;?></th>
            <th width="1%"><?php echo $rowtl2['exam']; ?></th>
            <th width="1%"><?php echo $rowtl2['exam']; ?></th>
            <th><?php echo $pref_tltotal;?></th>
            <th width="1%"><?php echo $pref_tl_quiztotal;?></th>
            <th width="1%"><?php echo $rowtl2['exam'];?></th>
            <th width="1%"><?php echo $pref_tltotal + $pref_tl_quiztotal + $rowtl2['exam'];?></th>
        </tr>
    </thead>
        <?php
        $grades=mysqli_query($conn,"SELECT * FROM student_prefinal WHERE subid='$subjectid' AND sy='$subjectsy'");
    ...
    <tbody class="text-center">
        ...
        <td id="g_green"><?php $preftotal = $pttotal + $quiztotal + $totalexam; echo round($preftotal);?></td>
    </tbody>
    <?php
    }
    ?>
