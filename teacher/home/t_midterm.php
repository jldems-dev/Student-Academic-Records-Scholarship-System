<table id="tblData" class="table table-bordered table-sm">
    <thead class="text-center">
        <tr>
            ...
            <th colspan="17"></th>
            <th width="1%" colspan="4">Computation</th>
        </tr>
        <tr>
        ...
        <tr style="background-color: yellow;">
            <?php  for($i = 1; $i <= 5; $i++){
            echo "<th width='1%'>".$rowtl1['pt'.$i]."</th>";
            }?>

            <th width="1%" ><?php echo $mid_tltotal;?></th>

            <?php  for($i = 1; $i <= 5; $i++){
            echo "<th width='1%'>".$rowtl1['quiz'.$i]."</th>";
            }?>

            <th width="1%"><?php echo $mid_tl_quiztotal;?></th>
            <th width="1%"><?php echo $rowtl1['exam']; ?></th>
            <th width="1%"><?php echo $rowtl1['exam']; ?></th>
            <th><?php echo $mid_tltotal;?></th>
            <th width="1%"><?php echo $mid_tl_quiztotal;?></th>
            <th width="1%"><?php echo $rowtl1['exam'];?></th>
            <th width="1%"><?php echo $mid_tltotal + $mid_tl_quiztotal + $rowtl1['exam'];?></th>
        </tr>
    </thead>
        <?php
        $grades=mysqli_query($conn,"SELECT * FROM student_midterm WHERE subid='$subjectid' AND sy='$subjectsy'");
    ...
    <tbody class="text-center">
        ...
        <?php
            ...
            echo '<td style="background-color: green;">'.$pttotal.'</td>';
            ...
            echo '<td style="background-color: green;">'.$quiztotal.'</td>';
        ?>
        <td><?php echo $rowgrades['exam1'];?></td>
        <td><?php echo $totalexam = $rowgrades['exam1']+50;?></td>
        <td style="background-color: green;"><?php $totalexam = ($rowgsheets['practical_exam']/100) * $totalexam; echo round($totalexam);?></td>
        <td><?php echo round($pttotal);?></td>
        <td><?php echo round($quiztotal);?></td>
        <td><?php echo round($totalexam);?></td>
        <td id="g_green"><?php $midtotal = $pttotal + $quiztotal + $totalexam; echo round($midtotal);?></td>
    </tbody>
    <?php
    }
    ?>
